...
class OT_Admin
{
  ...
  function option_tree_notice_post_locked( $post_id ) 
  {
    ...
    $last_user_name = $last_user ? $last_user->display_name : __('Somebody', 'claymore');
    $the_page = ( $_GET['page'] == 'option_tree' ) ? __('Theme Options', 'claymore') : __('Settings', 'claymore');
    
    $message = sprintf( __( 'Warning: %s is currently editing the %s.', 'claymore'), esc_html( $last_user_name ), $the_page );
